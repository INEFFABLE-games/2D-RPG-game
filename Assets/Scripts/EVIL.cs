...
public class EVIL : AbstractCharacter
{
    ...
    void OnDied(string message)
    {
        ...
        plrStats.exp += (100 * (10 + this.level - plrStats.level) / (10 + plrStats.level)) * 10;
        plrStats.reputation += 100;
        
        GameObject.FindGameObjectWithTag("Player").transform.Find("EndGameUI").gameObject.SetActive(true);
    ...
    {
        GameObject.FindGameObjectWithTag("RespawnManager").GetComponent<TimedRespawn>().Respawn(30f, respawnPosition, "DarkKnight", this.gameObject);
    }
